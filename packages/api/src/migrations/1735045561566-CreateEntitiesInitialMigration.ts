...
export class CreateEntitiesInitialMigration1735045561566 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`borrows\` (\`id\` int NOT NULL AUTO_INCREMENT, \`borrowedOn\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`returnedOn\` timestamp NULL, \`rating\` float NULL, \`userId\` int NULL, \`bookId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`books\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`author\` varchar(255) NOT NULL, \`publishedYear\` int NULL, \`averageRating\` float NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
